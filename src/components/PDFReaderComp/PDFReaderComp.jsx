...
import pdfJSCss from './JpPdfjs.css'
...
const PdfReaderComp = ({url = baseURL + '/pdfjs/test.pdf'}) => {
    ...
    const [pdfjsCssBlob, setPdfjsCssBlob] = useState('');
        ...
        return ReactDOM.createPortal(
            <div>
                ...
                <div title={'annotation'} >
                    <AnnotationIcon size={'1.5rem'} color={'#4A4A4B'}
                        className={'AnnotationIcon'} onClick={e=>{
...
// Open PDF file data using Uint8Array instead of URL
            await viewerApp.open({data: await Utils.urlToUint8Array(url)})
    ...
    //     height: '100vh',
    //     width: '100vw',
    // }} viewer-path={baseURL+'/pdfjs/pdfjs-4.0.189-dist'}>
